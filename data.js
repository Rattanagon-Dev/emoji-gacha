// --- OFFICIAL UNICODE NAMES & EXPANDED POOLS ---
// --- SECRET RARE (SCR) UNIVERSAL SYSTEM RELICS ---
const SCR_POOL = [
  { emoji: '💠', name: 'Diamond with a Dot' },
  { emoji: '🏔️', name: 'Snow-Capped Mountain' },
  { emoji: '💻', name: 'Laptop Computer' },
  { emoji: '⚙️', name: 'Gear System' },
  { emoji: '👁️‍🗨️', name: 'Eye in Speech Bubble' },
  { emoji: '⏳', name: 'Hourglass Done' },
  { emoji: '✨', name: 'Cosmic Sparkles' },
  { emoji: '⛏️', name: 'Quantum Pickaxe' },
  { emoji: '🔒', name: 'Cipher Lock' },
  { emoji: '🕒', name: 'Chrono Clock' },
  { emoji: '🛠️', name: 'Developer Rig Tools' }
];

const POOL = {
  UR: [
    { emoji: '💎', name: 'Gem Stone' },
    { emoji: '👑', name: 'Crown' },
    { emoji: '🌌', name: 'Milky Way' },
    { emoji: '🪐', name: 'Ringed Planet' },
    { emoji: '🦄', name: 'Unicorn' },
    { emoji: '🕊️', name: 'Dove' },
    { emoji: '🛸', name: 'Flying Saucer' },
    { emoji: '🪽', name: 'Wing' },
    { emoji: '⛩️', name: 'Shinto Shrine' },
    { emoji: '🌠', name: 'Shooting Star' },
    { emoji: '🎆', name: 'Fireworks' },
    { emoji: '🪬', name: 'Hamsa' },
    { emoji: '⚜️', name: 'Fleur-de-lis' }
  ],
  SSR: [
    { emoji: '🐉', name: 'Dragon' },
    { emoji: '⚔️', name: 'Crossed Swords' },
    { emoji: '🛡️', name: 'Shield' },
    { emoji: '🏹', name: 'Bow and Arrow' },
    { emoji: '⚡', name: 'High Voltage' },
    { emoji: '❄️', name: 'Snowflake' },
    { emoji: '🔥', name: 'Fire' },
    { emoji: '🌙', name: 'Crescent Moon' },
    { emoji: '☀️', name: 'Sun' },
    { emoji: '🗝️', name: 'Old Key' },
    { emoji: '🧿', name: 'Nazar Amulet' },
    { emoji: '💀', name: 'Skull' },
    { emoji: '🤖', name: 'Robot' },
    { emoji: '🧬', name: 'DNA' },
    { emoji: '🏰', name: 'Castle' },
    { emoji: '🦁', name: 'Lion' },
    { emoji: '🦅', name: 'Eagle' },
    { emoji: '🦈', name: 'Shark' },
    { emoji: '🏆', name: 'Trophy' },
    { emoji: '🎯', name: 'Bullseye' },
    { emoji: '🎪', name: 'Circus Tent' },
    { emoji: '🏎️', name: 'Racing Car' },
    { emoji: '🚀', name: 'Rocket' },
    { emoji: '🎲', name: 'Game Die' },
    { emoji: '🎸', name: 'Guitar' },
    { emoji: '⚓', name: 'Anchor' },
    { emoji: '🌋', name: 'Volcano' },
    { emoji: '🏵️', name: 'Rosette' },
    { emoji: '🪄', name: 'Magic Wand' }
  ],
  SR: [
    { emoji: '🐺', name: 'Wolf' },
    { emoji: '🐍', name: 'Snake' },
    { emoji: '🦊', name: 'Fox' },
    { emoji: '🐙', name: 'Octopus' },
    { emoji: '🦂', name: 'Scorpion' },
    { emoji: '🦉', name: 'Owl' },
    { emoji: '🧪', name: 'Test Tube' },
    { emoji: '📜', name: 'Scroll' },
    { emoji: '💍', name: 'Ring' },
    { emoji: '💣', name: 'Bomb' },
    { emoji: '🦇', name: 'Bat' },
    { emoji: '🦚', name: 'Peacock' },
    { emoji: '🦋', name: 'Butterfly' },
    { emoji: '🦔', name: 'Hedgehog' },
    { emoji: '🐢', name: 'Turtle' },
    { emoji: '🐬', name: 'Dolphin' },
    { emoji: '🦖', name: 'T-Rex' },
    { emoji: '🦧', name: 'Orangutan' },
    { emoji: '🦩', name: 'Flamingo' },
    { emoji: '🎋', name: 'Tanabata Tree' },
    { emoji: '🗡️', name: 'Dagger' },
    { emoji: '🪓', name: 'Axe' },
    { emoji: '🪃', name: 'Boomerang' },
    { emoji: '🎣', name: 'Fishing Pole' },
    { emoji: '🪙', name: 'Coin' },
    { emoji: '🪞', name: 'Mirror' },
    { emoji: '🪔', name: 'Diya Lamp' },
    { emoji: '🧲', name: 'Magnet' },
    { emoji: '🪁', name: 'Kite' },
    { emoji: '🏺', name: 'Amphora' },
    { emoji: '🪕', name: 'Banjo' },
    { emoji: '🎈', name: 'Balloon' },
    { emoji: '🎭', name: 'Performing Arts' },
    { emoji: '🕯️', name: 'Candle' },
    { emoji: '🧭', name: 'Compass' }
  ],
  R: [
    { emoji: '🍎', name: 'Red Apple' },
    { emoji: '🍌', name: 'Banana' },
    { emoji: '🍒', name: 'Cherries' },
    { emoji: '🍇', name: 'Grapes' },
    { emoji: '🍄', name: 'Mushroom' },
    { emoji: '🥕', name: 'Carrot' },
    { emoji: '🍪', name: 'Cookie' },
    { emoji: '🍉', name: 'Watermelon' },
    { emoji: '🍕', name: 'Pizza' },
    { emoji: '🍩', name: 'Doughnut' },
    { emoji: '☕', name: 'Hot Beverage' },
    { emoji: '🥑', name: 'Avocado' },
    { emoji: '🍔', name: 'Hamburger' },
    { emoji: '🍙', name: 'Rice Ball' },
    { emoji: '🍣', name: 'Sushi' },
    { emoji: '🍓', name: 'Strawberry' },
    { emoji: '🍍', name: 'Pineapple' },
    { emoji: '🥭', name: 'Mango' },
    { emoji: '🥥', name: 'Coconut' },
    { emoji: '🌽', name: 'Ear of Corn' },
    { emoji: '🥦', name: 'Broccoli' },
    { emoji: '🥐', name: 'Croissant' },
    { emoji: '🥞', name: 'Pancakes' },
    { emoji: '🍟', name: 'French Fries' },
    { emoji: '🌭', name: 'Hot Dog' },
    { emoji: '🥪', name: 'Sandwich' },
    { emoji: '🌮', name: 'Taco' },
    { emoji: '🍦', name: 'Soft Ice Cream' },
    { emoji: '🍰', name: 'Shortcake' },
    { emoji: '🍫', name: 'Chocolate Bar' },
    { emoji: '🍵', name: 'Teacup Without Handle' },
    { emoji: '🥤', name: 'Cup with Straw' },
    { emoji: '🥨', name: 'Pretzel' },
    { emoji: '🍿', name: 'Popcorn' },
    { emoji: '🍊', name: 'Tangerine' }
  ]
};

// --- FACES BANNER POOL (2nd Banner Theme) ---
const FACES_POOL = {
  UR: [
    { emoji: '🤩', name: 'Star-Struck' },
    { emoji: '🥳', name: 'Partying Face' },
    { emoji: '🤯', name: 'Exploding Head' },
    { emoji: '👽', name: 'Alien' },
    { emoji: '🥸', name: 'Disguised Face' },
    { emoji: '🫠', name: 'Melting Face' },
    { emoji: '👻', name: 'Ghost' },
    { emoji: '😈', name: 'Smiling Face with Horns' },
    { emoji: '🫥', name: 'Dotted Line Face' },
    { emoji: '😵‍💫', name: 'Face with Spiral Eyes' },
    { emoji: '🤠', name: 'Cowboy Hat Face' },
    { emoji: '👾', name: 'Alien Monster' }
  ],
  SSR: [
    { emoji: '🥶', name: 'Cold Face' },
    { emoji: '🥵', name: 'Hot Face' },
    { emoji: '🤪', name: 'Zany Face' },
    { emoji: '🤢', name: 'Nauseated Face' },
    { emoji: '🤮', name: 'Face Vomiting' },
    { emoji: '🥴', name: 'Woozy Face' },
    { emoji: '🤧', name: 'Sneezing Face' },
    { emoji: '🤒', name: 'Face with Thermometer' },
    { emoji: '🤕', name: 'Face with Head-Bandage' },
    { emoji: '🥺', name: 'Pleading Face' },
    { emoji: '😱', name: 'Face Screaming in Fear' },
    { emoji: '🥱', name: 'Yawning Face' },
    { emoji: '😷', name: 'Face with Medical Mask' },
    { emoji: '🤫', name: 'Shushing Face' },
    { emoji: '🤭', name: 'Face with Hand Over Mouth' }
  ],
  SR: [
    { emoji: '😎', name: 'Smiling Face with Sunglasses' },
    { emoji: '🤓', name: 'Nerd Face' },
    { emoji: '🧐', name: 'Face with Monocle' },
    { emoji: '😏', name: 'Smirking Face' },
    { emoji: '😌', name: 'Relieved Face' },
    { emoji: '😔', name: 'Pensive Face' },
    { emoji: '😴', name: 'Sleeping Face' },
    { emoji: '🙄', name: 'Face with Rolling Eyes' },
    { emoji: '😤', name: 'Face with Steam From Nose' },
    { emoji: '😡', name: 'Pouting Face' },
    { emoji: '😠', name: 'Angry Face' },
    { emoji: '🤬', name: 'Face with Symbols on Mouth' },
    { emoji: '😭', name: 'Loudly Crying Face' },
    { emoji: '😢', name: 'Crying Face' },
    { emoji: '😰', name: 'Anxious Face with Sweat' },
    { emoji: '😨', name: 'Fearful Face' },
    { emoji: '🥲', name: 'Smiling Face with Tear' },
    { emoji: '🤗', name: 'Hugging Face' }
  ],
  R: [
    { emoji: '😀', name: 'Grinning Face' },
    { emoji: '😃', name: 'Grinning Face with Big Eyes' },
    { emoji: '😄', name: 'Grinning Face with Smiling Eyes' },
    { emoji: '😁', name: 'Beaming Face' },
    { emoji: '😆', name: 'Grinning Squinting Face' },
    { emoji: '😅', name: 'Grinning Face with Sweat' },
    { emoji: '🤣', name: 'Rolling on the Floor Laughing' },
    { emoji: '😂', name: 'Face with Tears of Joy' },
    { emoji: '🙂', name: 'Slightly Smiling Face' },
    { emoji: '🙃', name: 'Upside-Down Face' },
    { emoji: '😉', name: 'Winking Face' },
    { emoji: '😊', name: 'Smiling Face with Smiling Eyes' },
    { emoji: '😇', name: 'Smiling Face with Halo' },
    { emoji: '🥰', name: 'Smiling Face with Hearts' },
    { emoji: '😍', name: 'Heart Eyes' },
    { emoji: '😘', name: 'Face Blowing a Kiss' },
    { emoji: '😗', name: 'Kissing Face' },
    { emoji: '😙', name: 'Kissing Face with Smiling Eyes' },
    { emoji: '😋', name: 'Face Savoring Food' },
    { emoji: '😛', name: 'Face with Tongue' }
  ]
};

// --- ALL-STAR COMBINED POOL (Universal Banner) ---
const ALL_POOL = {
  UR: [...POOL.UR, ...FACES_POOL.UR],
  SSR: [...POOL.SSR, ...FACES_POOL.SSR],
  SR: [...POOL.SR, ...FACES_POOL.SR],
  R: [...POOL.R, ...FACES_POOL.R]
};

const BANNER_POOLS = { cosmos: POOL, faces: FACES_POOL, all: ALL_POOL };

const BANNERS = [
  {
    id: 'cosmos',
    icon: '💠',
    tabLabel: 'Celestial Cosmos',
    title: 'Celestial Cosmos',
    tagline: '10-Pull guarantees SR or higher! Guaranteed SSR at 40, UR at 100 pulls.',
    ringClass: 'border-pink-400/50 shadow-pink-500/20',
    badgeClass: 'bg-pink-500/20 border-pink-400/40 text-pink-300'
  },
  {
    id: 'faces',
    icon: '🥳',
    tabLabel: 'Feelings Parade',
    title: 'Feelings Parade',
    tagline: 'A rotating cast of expressions! Same guarantees, brand-new faces to collect.',
    ringClass: 'border-amber-400/50 shadow-amber-500/20',
    badgeClass: 'bg-amber-500/20 border-amber-400/40 text-amber-300'
  },
  {
    id: 'all',
    icon: '🌌',
    tabLabel: 'Omniverse All-Stars',
    title: 'Omniverse All-Stars',
    tagline: 'All emojis from Cosmos and Feelings Parade combined in one massive pool!',
    ringClass: 'border-fuchsia-400/50 shadow-fuchsia-500/20',
    badgeClass: 'bg-fuchsia-500/20 border-fuchsia-400/40 text-fuchsia-300'
  }
];

const STAR_REQUIREMENTS = {
  1: 10, 2: 20, 3: 30, 4: 50, 5: 70,
  6: 90, 7: 120, 8: 150, 9: 200, 10: 300
};

const DUPLICATE_SHARDS = { 'SCR': 50, 'UR': 35, 'SSR': 20, 'SR': 10, 'R': 5 };

const PROMO_CODES = {
  '681210656': 'DEV_TOOL_ACTIVATED',
  'INFLATION': 100000,
  'LUCKY777': 77777,
  'JAIMINI': 50000,
  'WELCOME': 25000,
  'EMOJIGACHA': 15000,
  'BENDEV': 999999,
  'SECRETGEMS': 30000,
  'XFOLLOWER': 50000
};

// --- AVATAR PROFILES & FRAMES ---
const AVATAR_FRAMES = [
  { id: 'default', name: 'Default Slate', class: 'avatar-frame-default', req: 'Basic starter frame' },
  { id: 'gold', name: 'Golden Aura', class: 'avatar-frame-gold', req: 'Own 10 unique SSR emojis' },
  { id: 'prismatic', name: 'Prismatic Neon', class: 'avatar-frame-prismatic', req: 'Own 5 unique UR emojis' },
  { id: 'abyssal', name: 'Abyssal Glitch', class: 'avatar-frame-abyssal', req: 'Discover any SCR relic' }
];

const ACHIEVEMENTS = [
  { id: 'first_pull', icon: '🎉', name: 'First Steps', desc: 'Complete your first summon.', check: s => s.totalPulls >= 1 },
  { id: 'first_ur', icon: '🌟', name: 'First UR!', desc: 'Obtain your first UR item.', check: s => Object.values(s.inventory).some(i => i.tier === 'UR') },
  { id: 'first_scr', icon: '🌌', name: 'Cosmic Singularity', desc: 'Discover your first Secret Rare (SCR) relic.', check: s => s.hasUnlockedScr || Object.values(s.inventory).some(i => i.tier === 'SCR') },
  { id: 'custom_identity', icon: '🏷️', name: 'Identity Established', desc: 'Set a custom Summoner username.', check: s => s.username && s.username !== 'Summoner' },
  { id: 'collector_10', icon: '🎒', name: 'Collector I', desc: 'Own 10 unique emojis.', check: s => Object.keys(s.inventory).length >= 10 },
  { id: 'collector_30', icon: '📦', name: 'Collector II', desc: 'Own 30 unique emojis.', check: s => Object.keys(s.inventory).length >= 30 },
  { id: 'collector_60', icon: '🏛️', name: 'Collector III', desc: 'Own 60 unique emojis.', check: s => Object.keys(s.inventory).length >= 60 },
  { id: 'ur_hoarder', icon: '👑', name: 'UR Hoarder', desc: 'Own 5 unique UR items.', check: s => Object.values(s.inventory).filter(i => i.tier === 'UR').length >= 5 },
  { id: 'rainbow_master', icon: '🌈', name: 'Dimension Master', desc: 'Ascend any emoji to Rainbow Star.', check: s => Object.values(s.inventory).some(i => i.isRainbow) },
  { id: 'high_roller', icon: '🎰', name: 'High Roller', desc: 'Perform a 10x Summon, ten times.', check: s => (s.tenPullSummons || 0) >= 10 },
  { id: 'dedicated', icon: '🔥', name: 'Dedicated Summoner', desc: 'Reach 500 total pulls.', check: s => s.totalPulls >= 500 }
];

// --- BILINGUAL UI DICTIONARY (Auto / English) ---
const UI_TRANSLATIONS = {
  th: {
    nav_share: 'แชร์',
    nav_codex: 'สมุดภาพ',
    nav_rates: 'เรตสุ่ม',
    nav_calendar: 'ปฏิทิน',
    nav_log: 'ประวัติ',
    nav_awards: 'รางวัล',
    nav_settings: 'ตั้งค่า',
    banner_pull1: 'สุ่ม 1x',
    banner_pull10: 'สุ่ม 10x (การันตี SR+)',
    banner_pickup_btn: 'เลือกเป้าหมาย ➔',
    banner_pickup_none: 'ไม่มี (เรตมาตรฐาน)',
    collection_title: 'คลังสะสม',
    collection_hint: '💡 แตะการ์ดเพื่ออัปดาว หรือ 📌 ปักหมุดไว้บนสุด!',
    promote_all: 'เลื่อนขั้นทั้งหมด',
    promote_all_10: 'เลื่อนขั้น (สู่ 10★)',
    promote_all_rainbow: '🌈 จุติรุ้งทั้งหมด',
    quest_header: 'ภารกิจประจำวัน',
    quest_daily: 'เช็คอินรายวัน',
    quest_pulls: 'สุ่มครบ 10 ครั้ง',
    quest_promote: 'เลื่อนขั้นดาว 1 ครั้ง',
    quest_mine: 'ขุดแร่คริสตัล',
    btn_mine: '⛏️ แตะเพื่อขุดแร่!',
    btn_claim: 'รับรางวัล',
    btn_claimed: 'รับแล้ว',
    redeem_title: 'แลกโค้ดรางวัล',
    redeem_hint: 'กรอกโค้ดลับเพื่อรับเพชรฟรี!',
    redeem_btn: 'แลกรับ',
    filter_all_sets: 'ทุกเซ็ต',
    out_of_gems_title: 'เพชรไม่เพียงพอ!',
    out_of_gems_desc: 'เพชรไม่พอสำหรับการสุ่มครั้งนี้ ทำภารกิจรายวัน ขุดแร่ หรือจับเพชรตกในโหมดพักจอเพื่อรับเพชรเพิ่มนะ',
    out_of_gems_btn: '📜 ไปที่ภารกิจ',
    avatar_modal_title: 'ตั้งค่าโปรไฟล์ & มาสคอต',
    avatar_username_label: 'ชื่อผู้เล่น (Username)',
    avatar_frame_label: 'กรอบรูปโปรไฟล์'
  },
  en: {
    nav_share: 'Share',
    nav_codex: 'Codex',
    nav_rates: 'Rates',
    nav_calendar: 'Calendar',
    nav_log: 'Log',
    nav_awards: 'Awards',
    nav_settings: 'Settings',
    banner_pull1: 'Pull 1x',
    banner_pull10: 'Pull 10x (SR+ Guaran.)',
    banner_pickup_btn: 'Select Target ➔',
    banner_pickup_none: 'None (Standard Rates)',
    collection_title: 'Collection',
    collection_hint: '💡 Tap any card to upgrade stars or 📌 Pin to Top!',
    promote_all: 'Promote All',
    promote_all_10: 'Promote All (to 10★)',
    promote_all_rainbow: '🌈 Ascend All (Rainbow)',
    quest_header: 'Daily & Normal Quests',
    quest_daily: 'Daily Login',
    quest_pulls: 'Daily 10 Pulls',
    quest_promote: 'Daily Star Promote',
    quest_mine: 'Mine Crystal',
    btn_mine: '⛏️ Tap to Mine!',
    btn_claim: 'Claim',
    btn_claimed: 'Claimed',
    redeem_title: 'Redeem Code',
    redeem_hint: 'Enter secret codes to claim free Gems!',
    redeem_btn: 'Redeem',
    filter_all_sets: 'All Sets',
    out_of_gems_title: 'Not Enough Gems!',
    out_of_gems_desc: "You don't have enough gems for this summon. Complete daily quests, mine crystals, or catch falling gems in Screensaver mode to earn more.",
    out_of_gems_btn: '📜 Go to Quests',
    avatar_modal_title: 'Profile & Mascot Studio',
    avatar_username_label: 'Summoner Username',
    avatar_frame_label: 'Avatar Profile Frame'
  }
};
